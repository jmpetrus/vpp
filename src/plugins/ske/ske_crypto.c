...
extern sgx_enclave_id_t e_enclave_id;
...
v8 *
ske_calc_prf (ske_sa_transform_t * tr, v8 * key, v8 * data)
{
...
u8 *
ske_calc_prfplus (ske_sa_transform_t * tr, u8 * key, u8 * seed, int len)
{
...
v8 *
ske_calc_integr (ske_sa_transform_t * tr, v8 * key, u8 * data, int len)
{
  v8 *r;
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  HMAC_CTX *hctx;
  ...
  ASSERT (tr->type == SKE_TRANSFORM_TYPE_INTEG);
...
v8 *
ske_decrypt_data (ske_sa_t * sa, u8 * data, int len)
{
#if 0
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  EVP_CIPHER_CTX *ctx;
  ...
  ret = Enclave_decrypt_data(e_enclave_id,
          sa->index_spi,
          sa->is_initiator,
          tr_encr->mode,
          block_size,
    ...
    if(ret != SGX_SUCCESS){
        clib_warning("Failed to decrypt_data(): ret=%d", ret);
    }
#if DEBUG_SUCCEED
    clib_warning("Succeed to decrypt_data()");
#endif
#if TIME_MEASUREMENT /* time measurement */
  end = clib_cpu_time_now();
  end_ns = unix_time_now_nsec();
  clib_warning("(TIME) Elapsed time (len=%d) : cycles=%llu, clock=%llu ns", 
                 len, end - start, end_ns - start_ns);
#endif
...
int
ske_encrypt_data (ske_sa_t * sa, v8 * src, u8 * dst)
{
#if 0
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  EVP_CIPHER_CTX *ctx;
  ...
  ret = Enclave_encrypt_data(e_enclave_id,
	  sa->index_spi,
          sa->is_initiator,
          tr_encr->mode,
          bs,
    ...
    if(ret != SGX_SUCCESS){
        clib_warning("Failed to encrypt_data(): ret=%d", ret);
    }
#if DEBUG_SUCCEED
    clib_warning("Succeed to encrypt_data()");
#endif
#if TIME_MEASUREMENT /* time measurement */
  end = clib_cpu_time_now();
  end_ns = unix_time_now_nsec();
  clib_warning("(TIME) Elapsed time (len=%d): cycles=%llu, clock=%llu ns", 
                 vec_len(src), end - start, end_ns - start_ns);
#endif
...
void
ske_generate_dh (ske_sa_t * sa, ske_sa_transform_t * t)
{
  int r;

  if (t->dh_group == SKE_DH_GROUP_MODP)
    {
#if 0
      DH *dh = DH_new ();
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  ...
  start_ns = unix_time_now_nsec();
    ...
    ret = Enclave_generate_dh(e_enclave_id,
        sa->is_initiator,
	sa->index_spi,
        t->key_len, 
        sa->i_dh_data, vec_len(sa->i_dh_data),
        sa->r_dh_data,  
        t->dh_p, t->dh_g
#if 1 /* DEBUG */
        , (sa->is_initiator ? sa->dh_private_key : sa->dh_shared_key)
#endif
    );
    if(ret != SGX_SUCCESS){
        clib_warning("Failed to generate_dh(): ret=%d", ret);
    }
#if TIME_MEASUREMENT /* time measurement */
  end = clib_cpu_time_now();
  end_ns = unix_time_now_nsec();
  ...
  else if (t->dh_group == SKE_DH_GROUP_ECP)
    {
...
void
ske_complete_dh (ske_sa_t * sa, ske_sa_transform_t * t)
{
#if 0
  int r;
#endif
  ...
  start_ns = unix_time_now_nsec();
     ...
     ret = Enclave_complete_dh(e_enclave_id,
	sa->index_spi,
        t->key_len, 
        sa->r_dh_data, vec_len(sa->r_dh_data),  
        t->dh_p, t->dh_g, 
        sa->dh_shared_key
    );
    if(ret != SGX_SUCCESS){
        clib_warning("Failed to complete_dh(): ret=%d", ret);
    }
#if DEBUG_SUCCEED
    else
        clib_warning("Succeed to complete_dh()");
#endif
#if TIME_MEASUREMENT /* time measurement */
  end = clib_cpu_time_now();
  end_ns = unix_time_now_nsec();
...
int
ske_verify_sign (EVP_PKEY * pkey, u8 * sigbuf, u8 * data)
{
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
...
u8 *
ske_calc_sign (EVP_PKEY * pkey, u8 * data)
{
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
  EVP_MD_CTX *md_ctx = EVP_MD_CTX_new ();
#else
...
ske_crypto_init (ske_main_t * km)
{
  ...
  tr->type = SKE_TRANSFORM_TYPE_ENCR;
  tr->encr_type = SKE_TRANSFORM_ENCR_TYPE_AES_CBC;
  tr->key_len = 256 / 8;
  tr->block_size = 128 / 8;
  tr->mode = EKE_TRANSFORM_ENCR_TYPE_AES_CBC_256;
  ...
  tr->key_len = 192 / 8;
  tr->block_size = 128 / 8;
  tr->mode = EKE_TRANSFORM_ENCR_TYPE_AES_CBC_192;
  ...
  tr->key_len = 128 / 8;
  tr->block_size = 128 / 8;
  tr->mode = EKE_TRANSFORM_ENCR_TYPE_AES_CBC_128;
  tr->cipher = EVP_aes_128_cbc ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_ENCR;
  tr->encr_type = SKE_TRANSFORM_ENCR_TYPE_AES_GCM_16;
  tr->key_len = 256 / 8;
  tr->block_size = 128 / 8;
  ...
  //PRF
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_PRF;
  tr->prf_type = SKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_256;
  tr->key_len = 256 / 8;
  tr->key_trunc = 256 / 8;
  tr->mode = EKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_256;
  tr->md = EVP_sha256 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_PRF;
  tr->prf_type = SKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_384;
  tr->key_len = 384 / 8;
  tr->key_trunc = 384 / 8;
  tr->mode = EKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_384;
  tr->md = EVP_sha384 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_PRF;
  tr->prf_type = SKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_512;
  tr->key_len = 512 / 8;
  tr->key_trunc = 512 / 8;
  tr->mode = EKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_512;
  tr->md = EVP_sha512 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_PRF;
  tr->prf_type = SKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA1;
  tr->key_len = 160 / 8;
  tr->key_trunc = 160 / 8;
  tr->mode = EKE_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA1;
  tr->md = EVP_sha1 ();

  //Integrity
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_INTEG;
  tr->integ_type = SKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_256_128;
  tr->key_len = 256 / 8;
  tr->key_trunc = 128 / 8;
  tr->mode = EKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_256_128;
  tr->md = EVP_sha256 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_INTEG;
  tr->integ_type = SKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_384_192;
  tr->key_len = 384 / 8;
  tr->key_trunc = 192 / 8;
  tr->mode = EKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_384_192;
  tr->md = EVP_sha384 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_INTEG;
  tr->integ_type = SKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_512_256;
  tr->key_len = 512 / 8;
  tr->key_trunc = 256 / 8;
  tr->mode = EKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA2_512_256;
  tr->md = EVP_sha512 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_INTEG;
  tr->integ_type = SKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA1_160;
  tr->key_len = 160 / 8;
  tr->key_trunc = 160 / 8;
  tr->mode = EKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA1_160;
  tr->md = EVP_sha1 ();

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_INTEG;
  tr->integ_type = SKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA1_96;
  tr->key_len = 160 / 8;
  tr->key_trunc = 96 / 8;
  tr->mode = EKE_TRANSFORM_INTEG_TYPE_AUTH_HMAC_SHA1_96;
  tr->md = EVP_sha1 ();

#if defined(OPENSSL_NO_CISCO_FECDH)
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_BRAINPOOL_512;
  tr->key_len = (512 * 2) / 8;
  tr->nid = NID_brainpoolP512r1;
  tr->dh_group = SKE_DH_GROUP_ECP;

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_BRAINPOOL_384;
  tr->key_len = (384 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_BRAINPOOL_256;
  tr->key_len = (256 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_BRAINPOOL_224;
  tr->key_len = (224 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_ECP_224;
  tr->key_len = (224 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_ECP_521;
  tr->key_len = (528 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_ECP_384;
  tr->key_len = (384 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_ECP_256;
  tr->key_len = (256 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_ECP_192;
  tr->key_len = (192 * 2) / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_2048_256;
  tr->key_len = 2048 / 8;
  tr->dh_p = (const char *) &modp_dh_2048_256_prime;
  tr->dh_g = (const char *) &modp_dh_2048_256_generator;
  tr->dh_group = SKE_DH_GROUP_MODP;

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_2048_224;
  tr->key_len = 2048 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_1024_160;
  tr->key_len = 1024 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_8192;
  tr->key_len = 8192 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_6144;
  tr->key_len = 6144 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_4096;
  tr->key_len = 4096 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_3072;
  tr->key_len = 3072 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_2048;
  tr->key_len = 2048 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_1536;
  tr->key_len = 1536 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_1024;
  tr->key_len = 1024 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_DH;
  tr->dh_type = SKE_TRANSFORM_DH_TYPE_MODP_768;
  tr->key_len = 768 / 8;
  ...
  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_ESN;
  tr->esn_type = SKE_TRANSFORM_ESN_TYPE_ESN;

  vec_add2 (km->supported_transforms, tr, 1);
  tr->type = SKE_TRANSFORM_TYPE_ESN;
  tr->esn_type = SKE_TRANSFORM_ESN_TYPE_NO_ESN;
}
